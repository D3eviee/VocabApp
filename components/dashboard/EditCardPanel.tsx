...
import { useEditorStore, FlashcardDraft } from '@/store/use-editor-store';
...
export const EditCardPanel = ({ deckId, onBack }: { deckId: string, onBack: () => void }) => {
  ...
  useEffect(() => {
    ...
    if (card) {
      ...
      clonedCard.front = clonedCard.front || "";
      clonedCard.partOfSpeech = clonedCard.partOfSpeech || "";
      ...
      setFormData(clonedCard as FlashcardDraft);
    } else {
      setFormData(null);
    }
  ...
  return (
    <main className="relative flex-1 flex flex-col h-full overflow-y-auto bg-white lg:bg-transparent custom-scrollbar min-h-0">
      <div className="sticky top-0 z-20 flex items-center justify-between px-4 py-3 shrink-0 bg-white/90 lg:bg-[#F5F5F7]/90 backdrop-blur-md border-b border-gray-200/60">
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          <button
            ...
            className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors hover:cursor-pointer"
            title="Delete card"
          >
            <Trash2 size={20} />
          </button>
      ...
      {/* FORMULARZ (Bez zmian w strukturze) */}
