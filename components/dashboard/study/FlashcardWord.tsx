...
export const getPartOfSpeechColor = (pos?: string | null) => {
  ...
  switch (normalizedPos) {
    case 'noun': return 'bg-blue-700 text-blue-700';
...
export const FlashcardWord = ({ word, partOfSpeech, meanings }: FlashcardWordProps) => {
  return (
    <div className="w-full bg-[#F2F2F2] p-6 rounded-3xl">
      <div className="flex flex-col border-b border-[#D4D4D4] mb-4">
        <p className={`w-fit mb-2 px-2 py-0.5 rounded-md text-xs font-semibold uppercase tracking-wider ${getPartOfSpeechColor(partOfSpeech)}`}>{partOfSpeech?.replace('_', ' ') || 'noun'}</p>
        <h2 className="text-3xl font-bold text-[#111] mb-4">{word}</h2>
      </div>
        ...
        {meanings?.map((m: any, mIdx: number) => (
          <div key={m.id || mIdx} >
            <div className="flex flex-row gap-2">
              <div className={`w-1 shrink-0 rounded-xl ${getPartOfSpeechColor(partOfSpeech)}`} />
              <p className="text-15 font-semibold text-[#333]">{m.back}</p>
            </div>
